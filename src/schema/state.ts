 ...
 */
export const StateFromString: Schema.Schema<State, string> =
  Schema.transformOrFail(Schema.String, Schema.Literal(0, 1, 2, 3), {
    strict: true,
    decode: (s, _options, ast) => {
      if (!STATE_PATTERN.test(s)) {
        return ParseResult.fail(
          new ParseResult.Type(ast, s, `State must be 0-3, got "${s}"`)
        );
      }
