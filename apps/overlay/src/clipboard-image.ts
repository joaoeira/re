        ...
        new Promise<{ file?: string }>((resolve, reject) => {
          execFile(
            ...
            [
              ...
              "JavaScript",
              "-e",
              `ObjC.import("AppKit"); const value = $.NSPasteboard.generalPasteboard.stringForType("public.file-url"); value ? ObjC.unwrap(value) : "";`,
            ],
            { encoding: "utf8" },
            (error, output) =>
              error ? reject(error) : resolve(output.trim() ? { file: output.trim() } : {}),
          );
        }),
      catch: (error) =>
        new ClipboardImageReadError({
