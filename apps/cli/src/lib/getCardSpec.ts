...
const itemTypes = [QAType, ClozeType] as ReadonlyArray<
  ItemType<unknown, unknown, unknown>
>
...
export const getCardSpec = (
  queueItem: QueueItem
): Effect.Effect<CardSpec<Grade>, CardSpecError> =>
  Effect.gen(function* () {
    const result = yield* inferType(itemTypes, queueItem.item.content).pipe(
      Effect.mapError(
        (e) =>
          new CardSpecError(`Failed to parse item: ${e._tag}: ${e.message}`)
      )
    )
