...
import { DeckWriter, DeckWriterLive, DeckWriteError } from "../../src/services/DeckWriter";
...
describe("DeckWriter", () => {
  describe("updateCard", () => {
    ...
    it("updates correct item when multiple items have same content", async () => {
      ...
      const MockFS = FileSystem.layerNoop({
        readFileString: (path) => {
          if (path === "/duplicates.md") return Effect.succeed(duplicateDeckContent);
          return Effect.fail(
            new SystemError({
              reason: "NotFound",
              module: "FileSystem",
    ...
    it("fails with out-of-bounds item index", async () => {
      ...
      const result = await Effect.gen(function* () {
        const writer = yield* DeckWriter;
        return yield* writer.updateCard("/test.md", 999, 0, newCard).pipe(Effect.either);
      }).pipe(Effect.provide(TestLayer), Effect.runPromise);
    ...
    it("fails with out-of-bounds card index", async () => {
      ...
      const result = await Effect.gen(function* () {
        const writer = yield* DeckWriter;
        return yield* writer.updateCard("/test.md", 0, 999, newCard).pipe(Effect.either);
      }).pipe(Effect.provide(TestLayer), Effect.runPromise);
    ...
    it("fails when file doesn't exist", async () => {
      ...
      const result = await Effect.gen(function* () {
        const writer = yield* DeckWriter;
        return yield* writer.updateCard("/missing.md", 0, 0, newCard).pipe(Effect.either);
      }).pipe(Effect.provide(TestLayer), Effect.runPromise);
