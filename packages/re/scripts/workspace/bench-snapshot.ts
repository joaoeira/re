 ...
 * in packages/re/src/core.
 *
 * Usage (from packages/re):
 *   bun scripts/workspace/bench-snapshot.ts                  # all presets
 *   bun scripts/workspace/bench-snapshot.ts small            # ~50 decks / ~1k cards
 *   bun scripts/workspace/bench-snapshot.ts medium           # ~200 decks / ~10k cards
 *   bun scripts/workspace/bench-snapshot.ts large-files      # ~1k decks / ~50k cards
 *   bun scripts/workspace/bench-snapshot.ts large-cards      # ~200 decks / ~100k cards
 *   bun scripts/workspace/bench-snapshot.ts --vault <path>   # one-off run against a real vault
 *   bun scripts/workspace/bench-snapshot.ts --runs 5 --keep  # more passes, keep generated vault
 *
...
const usage = (): never => {
  console.error(
    "Usage: bun scripts/workspace/bench-snapshot.ts [small] [medium] [large-files] [large-cards] [--vault <path>] [--runs <n>] [--warmup <n>] [--keep]",
  );
  process.exit(1);
};
