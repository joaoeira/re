...
describe("NumericFieldFromString", () => {
  ...
  it.scoped("preserves trailing zeros", () =>
    Effect.gen(function* () {
      const result = yield* Schema.decodeUnknown(NumericFieldFromString)(
        "5.20"
      );
      assert.strictEqual(result.value, 5.2);
...
describe("StateFromString", () => {
  ...
  it.scoped("rejects 4", () =>
    Effect.gen(function* () {
      const result = yield* Schema.decodeUnknown(StateFromString)("4").pipe(
        Effect.flip
      );
      assert.ok(result);
    })
  ...
  it.scoped("rejects negative", () =>
    Effect.gen(function* () {
      const result = yield* Schema.decodeUnknown(StateFromString)("-1").pipe(
        Effect.flip
      );
      assert.ok(result);
    })
...
describe("LearningStepsFromString", () => {
  ...
  it.scoped("rejects negative", () =>
    Effect.gen(function* () {
      const result = yield* Schema.decodeUnknown(LearningStepsFromString)(
        "-1"
      ).pipe(Effect.flip);
      assert.ok(result);
    })
...
describe("LastReviewFromString", () => {
  ...
  it.scoped("fails to encode invalid Date", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Schema.encode(LastReviewFromString)(
        invalidDate
      ).pipe(Effect.flip);
      assert.ok(result);
    })
...
describe("ItemIdSchema", () => {
  ...
  it.scoped("rejects empty strings", () =>
    Effect.gen(function* () {
      const result = yield* Schema.decodeUnknown(ItemIdSchema)("").pipe(
        Effect.flip
      );
      assert.ok(result);
    })
