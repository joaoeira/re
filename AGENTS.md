...
- **Base UI Vite dep optimizer**: `vitest.config.ts` sets `optimizeDeps.entries` for the browser project to crawl `test/**/*.browser.test.tsx` and `src/renderer/src/**/*.{ts,tsx}`, so new `@base-ui/react/*` (or any other) sub-path imports are auto-discovered and pre-bundled at startup — no manual list maintenance needed. The `optimizeDeps.include` list is a backstop for known-needed packages. If you ever see hangs or `TypeError: Cannot read properties of null (reading 'useContext')` from Base UI, delete `node_modules/.vite/vitest/` to force a re-bundle; if the issue persists, add the package to `optimizeDeps.include`.
